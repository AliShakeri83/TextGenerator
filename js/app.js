...
let inputElem = document.querySelector('#amount')
...
btn.addEventListener ('click', function (event) {
  ...
  let randomIndexText = Math.floor(Math.random() * dummyText.length)

  if (inputElemValue<0 || inputElemValue>9 || inputElemValue==='' || isNaN(inputElemValue)) {
    result.innerHTML = dummyText[randomIndexText]
  } else {
    let sliceArray = dummyText.slice(0, inputElemValue)
